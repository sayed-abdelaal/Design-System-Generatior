import JSZip from "jszip";

import { FONT_OPTIONS } from "@/data/fonts";
import { resolveTokenReference } from "@/lib/generator";
import {
  GeneratedSystem,
  SCALE_STEPS,
  SEMANTIC_TOKEN_NAMES,
  ThemeSemanticTokens,
} from "@/types/design-system";

function semanticBlock(name: string, tokens: ThemeSemanticTokens, system: GeneratedSystem) {
  return SEMANTIC_TOKEN_NAMES.map((token) => {
    const cssName = token.replace(/[A-Z]/g, (match) => `-${match.toLowerCase()}`);
    const value = resolveTokenReference(tokens[token], system.palettes);

    return `  --ds-${cssName}: ${value};`;
  }).join("\n");
}

function paletteLines(system: GeneratedSystem) {
  return Object.entries(system.palettes).flatMap(([paletteName, scale]) =>
    SCALE_STEPS.map((step) => `  --ds-${paletteName}-${step}: ${scale[step]};`),
  ).join("\n");
}

function typographyLines(system: GeneratedSystem) {
  return [
    `  --ds-font-display: ${system.typography.displayFont};`,
    `  --ds-font-heading: ${system.typography.headingFont};`,
    `  --ds-font-body: ${system.typography.bodyFont};`,
    ...Object.entries(system.typography.scale).flatMap(([key, token]) => [
      `  --ds-type-${key}-size: ${token.size};`,
      `  --ds-type-${key}-line-height: ${token.lineHeight};`,
      `  --ds-type-${key}-weight: ${token.weight};`,
      ...(token.letterSpacing ? [`  --ds-type-${key}-tracking: ${token.letterSpacing};`] : []),
    ]),
  ].join("\n");
}

function chromeLines(system: GeneratedSystem) {
  return [
    ...Object.entries(system.radius).map(([key, value]) => `  --ds-radius-${key}: ${value};`),
    ...Object.entries(system.shadows).map(([key, value]) => `  --ds-shadow-${key}: ${value};`),
    ...Object.entries(system.foundations.insetShadows).map(([key, value]) => `  --ds-inset-shadow-${key}: ${value};`),
    ...Object.entries(system.foundations.dropShadows).map(([key, value]) => `  --ds-drop-shadow-${key}: ${value};`),
    ...Object.entries(system.foundations.blur).map(([key, value]) => `  --ds-blur-${key}: ${value};`),
    ...Object.entries(system.foundations.opacity).map(([key, value]) => `  --ds-opacity-${key}: ${value};`),
    ...Object.entries(system.foundations.easing).map(([key, value]) => `  --ds-ease-${key}: ${value};`),
    ...Object.entries(system.foundations.durations).map(([key, value]) => `  --ds-duration-${key}: ${value};`),
    ...Object.entries(system.foundations.animations).map(([key, value]) => `  --ds-animate-${key}: ${value};`),
    ...Object.entries(system.foundations.breakpoints).map(([key, value]) => `  --ds-breakpoint-${key}: ${value};`),
    ...Object.entries(system.foundations.containers).map(([key, value]) => `  --ds-container-${key}: ${value};`),
    ...Object.entries(system.foundations.borderWidths).map(([key, value]) => `  --ds-border-width-${key}: ${value};`),
    ...Object.entries(system.foundations.spacing).map(([key, value]) => `  --ds-spacing-${key}: ${value};`),
    ...Object.entries(system.foundations.fontWeights).map(([key, value]) => `  --ds-font-weight-${key}: ${value};`),
    ...Object.entries(system.foundations.tracking).map(([key, value]) => `  --ds-tracking-${key}: ${value};`),
    ...Object.entries(system.foundations.leading).map(([key, value]) => `  --ds-leading-${key}: ${value};`),
    ...Object.entries(system.foundations.aspectRatios).map(([key, value]) => `  --ds-aspect-${key}: ${value};`),
    ...Object.entries(system.foundations.zIndex).map(([key, value]) => `  --ds-z-${key}: ${value};`),
    `  --ds-icon-size: ${system.icons.defaultSize}px;`,
    `  --ds-icon-stroke-width: ${system.icons.strokeWidth};`,
    `  --ds-icon-color-behavior: ${system.icons.colorBehavior};`,
    ...Object.entries(system.icons.semanticUsage).map(([key, value]) => `  --ds-icon-${key}: ${resolveTokenReference(value, system.palettes)};`),
    `  --ds-density: ${system.density};`,
  ].join("\n");
}

function markdownTable(rows: Array<Array<string | number>>) {
  if (!rows.length) {
    return "";
  }

  const [header, ...body] = rows;
  const divider = header.map(() => "---");

  return [header, divider, ...body]
    .map((row) => `| ${row.join(" | ")} |`)
    .join("\n");
}

function semanticTable(tokens: ThemeSemanticTokens, system: GeneratedSystem) {
  return markdownTable([
    ["Role", "Token", "Resolved value"],
    ...SEMANTIC_TOKEN_NAMES.map((token) => [
      token,
      tokens[token],
      resolveTokenReference(tokens[token], system.palettes),
    ]),
  ]);
}

function paletteAnchorTable(system: GeneratedSystem) {
  return markdownTable([
    ["Palette", "50", "500", "950"],
    ...Object.entries(system.palettes).map(([name, scale]) => [
      name,
      scale["50"],
      scale["500"],
      scale["950"],
    ]),
  ]);
}

function typographyTable(system: GeneratedSystem) {
  return markdownTable([
    ["Role", "Size", "Line height", "Weight", "Tracking"],
    ...Object.entries(system.typography.scale).map(([role, token]) => [
      role,
      token.size,
      token.lineHeight,
      token.weight,
      token.letterSpacing ?? "normal",
    ]),
  ]);
}

function componentSummaryTable(system: GeneratedSystem) {
  return markdownTable([
    ["Component", "Primary recipe guidance"],
    ...Object.entries(system.components).slice(0, 40).map(([name, recipe]) => [
      name,
      Object.entries(recipe as Record<string, unknown>)
        .slice(0, 4)
        .map(([key, value]) => `${key}: ${typeof value === "object" ? "configured" : String(value)}`)
        .join("; "),
    ]),
  ]);
}

export function buildThemeCss(system: GeneratedSystem) {
  return `:root {\n${paletteLines(system)}\n${typographyLines(system)}\n${chromeLines(system)}\n${semanticBlock("light", system.lightTokens, system)}\n}\n\n[data-theme="dark"] {\n${semanticBlock("dark", system.darkTokens, system)}\n}\n`;
}

export function buildThemeLayerCss(system: GeneratedSystem) {
  const paletteTokens = Object.entries(system.palettes)
    .flatMap(([paletteName, scale]) => SCALE_STEPS.map((step) => `  --color-${paletteName}-${step}: ${scale[step]};`))
    .join("\n");

  return `@theme static inline {\n  --color-background: var(--ds-background);\n  --color-foreground: var(--ds-foreground);\n  --color-surface: var(--ds-surface);\n  --color-surface-elevated: var(--ds-surface-elevated);\n  --color-text-primary: var(--ds-text-primary);\n  --color-text-secondary: var(--ds-text-secondary);\n  --color-text-muted: var(--ds-text-muted);\n  --color-border-default: var(--ds-border-default);\n  --color-border-strong: var(--ds-border-strong);\n  --color-action-primary: var(--ds-action-primary);\n  --color-action-secondary: var(--ds-action-secondary);\n  --color-focus-ring: var(--ds-focus-ring);\n  --color-success: var(--ds-success);\n  --color-warning: var(--ds-warning);\n  --color-danger: var(--ds-danger);\n  --color-info: var(--ds-info);\n  --color-attention: var(--ds-attention);\n  --color-highlight: var(--ds-highlight);\n  --font-sans: var(--ds-font-body);\n  --font-display: var(--ds-font-display);\n${paletteTokens}\n${Object.entries(system.foundations.spacing).map(([key, value]) => `  --spacing-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.fontWeights).map(([key, value]) => `  --font-weight-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.tracking).map(([key, value]) => `  --tracking-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.leading).map(([key, value]) => `  --leading-${key}: ${value};`).join("\n")}\n${Object.entries(system.radius).map(([key, value]) => `  --radius-${key}: ${value};`).join("\n")}\n${Object.entries(system.shadows).map(([key, value]) => `  --shadow-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.borderWidths).map(([key, value]) => `  --border-width-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.insetShadows).map(([key, value]) => `  --inset-shadow-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.dropShadows).map(([key, value]) => `  --drop-shadow-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.blur).map(([key, value]) => `  --blur-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.opacity).map(([key, value]) => `  --opacity-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.breakpoints).map(([key, value]) => `  --breakpoint-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.containers).map(([key, value]) => `  --container-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.aspectRatios).map(([key, value]) => `  --aspect-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.easing).map(([key, value]) => `  --ease-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.durations).map(([key, value]) => `  --duration-${key}: ${value};`).join("\n")}\n${Object.entries(system.foundations.animations).map(([key, value]) => `  --animate-${key}: ${value};`).join("\n")}\n}\n\n@theme {\n  @keyframes fade-in {\n    0% { opacity: 0; }\n    100% { opacity: 1; }\n  }\n\n  @keyframes rise-in {\n    0% { opacity: 0; transform: translateY(8px); }\n    100% { opacity: 1; transform: translateY(0); }\n  }\n\n  @keyframes pulse-soft {\n    0%, 100% { opacity: 1; }\n    50% { opacity: 0.7; }\n  }\n}\n`;
}

export function buildTokensJson(system: GeneratedSystem, brandName: string) {
  const display = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.displayFont)?.family ?? "Custom";
  const heading = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.headingFont)?.family ?? "Custom";
  const body = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.bodyFont)?.family ?? "Custom";

  return JSON.stringify(
    {
      meta: {
        product: "Design System Generator",
        brandName,
        colorAdjustmentMode: system.colorAdjustmentMode,
      },
      palettes: system.palettes,
      customPalettes: system.customPalettes,
      semantics: {
        light: system.lightTokens,
        dark: system.darkTokens,
      },
      typography: {
        displayFont: display,
        headingFont: heading,
        bodyFont: body,
        scale: system.typography.scale,
      },
      icons: system.icons,
      foundations: system.foundations,
      utilities: system.utilities,
      utilityCoverage: system.utilityCoverage,
      components: system.components,
      screens: system.screens,
      radius: system.radius,
      shadows: system.shadows,
      density: system.density,
    },
    null,
    2,
  );
}

export function buildComponentsJson(system: GeneratedSystem, brandName: string) {
  return JSON.stringify(
    {
      meta: {
        product: "Design System Generator",
        brandName,
        colorAdjustmentMode: system.colorAdjustmentMode,
      },
      components: system.components,
      customPalettes: system.customPalettes,
      icons: system.icons,
      utilities: system.utilities,
      utilityCoverage: system.utilityCoverage,
      foundations: system.foundations,
      screens: system.screens,
    },
    null,
    2,
  );
}

export function buildSessionJson(system: GeneratedSystem, brandName: string) {
  return JSON.stringify(
    {
      version: 1,
      meta: {
        product: "Design System Generator",
        brandName,
        colorAdjustmentMode: system.colorAdjustmentMode,
      },
      system,
    },
    null,
    2,
  );
}

export function buildReadme(system: GeneratedSystem, brandName: string) {
  const paletteCount = Object.keys(system.palettes).length;
  const customPaletteNames = system.customPalettes.map((palette) => palette.name).join(", ") || "None";

  return `# ${brandName} Design Tokens\n\nThis package was generated by Design System Generator.\n\n## Snapshot\n- Total palettes: ${paletteCount}\n- Custom palettes: ${customPaletteNames}\n- Density mode: ${system.density}\n- Color handling: ${system.colorAdjustmentMode}\n\n## Files\n- \`theme.css\` defines raw palette variables, semantic tokens, typography, icon variables, radius, shadows, and density.\n- \`theme-layer.css\` maps those variables into a reusable theme layer with \`@theme\`.\n- \`tokens.json\` provides palettes, semantics, foundations, icon settings, utility coverage, screen presets, and density.\n- \`components.json\` provides component recipes, icon settings, utilities, coverage metadata, and screens for tooling.\n- \`design-system-session.json\` stores the full generated system for round-tripping.\n\n## Usage\n1. Import \`theme.css\` after your app styles.\n2. Import \`theme-layer.css\` where your project reads theme variables.\n3. Toggle dark mode by adding \`data-theme="dark"\` on a parent element.\n4. Reload \`design-system-session.json\` in the app if you want to continue editing later.\n`;
}

export function buildDesignMd(system: GeneratedSystem, brandName: string) {
  const display = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.displayFont)?.family ?? system.typography.displayFont;
  const heading = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.headingFont)?.family ?? system.typography.headingFont;
  const body = FONT_OPTIONS.find((font) => font.cssVariable === system.typography.bodyFont)?.family ?? system.typography.bodyFont;

  return `# ${brandName} Design System Instructions

Use this file as the source of truth when implementing product UI for ${brandName}. Follow the tokens and recipes below instead of inventing one-off styles.

## Implementation Priorities
- Use semantic roles first: background, surface, text, border, action, focus, success, warning, danger, info, attention, and highlight.
- Prefer component recipes from \`components.json\` when building reusable UI.
- Preserve the selected density mode: \`${system.density}\`.
- Preserve color handling mode: \`${system.colorAdjustmentMode}\`.
- Support light and dark mode intentionally. Do not invert colors automatically.
- Keep accessibility visible: respect focus styles, touch target minimums, and contrast warnings.

## Brand Direction
- Style direction: \`${system.styleDirection}\`
- Display font: ${display}
- Heading font: ${heading}
- Body font: ${body}
- Icon behavior: ${system.icons.colorBehavior}
- Default icon size: ${system.icons.defaultSize}px
- Default icon stroke: ${system.icons.strokeWidth}

## Palette Anchors
${paletteAnchorTable(system)}

## Light Theme Semantic Tokens
${semanticTable(system.lightTokens, system)}

## Dark Theme Semantic Tokens
${semanticTable(system.darkTokens, system)}

## Typography Scale
${typographyTable(system)}

## Layout, Spacing, Radius, and Elevation
- Density: ${system.density}
- Default radius key: ${system.utilities.layout.defaultRadius}
- Border radius scale: ${Object.entries(system.radius).map(([key, value]) => `${key}=${value}`).join(", ")}
- Shadow scale: ${Object.entries(system.shadows).map(([key, value]) => `${key}=${value}`).join(", ")}
- Core spacing scale: ${Object.entries(system.foundations.spacing).map(([key, value]) => `${key}=${value}`).join(", ")}
- Container widths: ${Object.entries(system.foundations.containers).map(([key, value]) => `${key}=${value}`).join(", ")}
- Breakpoints: ${Object.entries(system.foundations.breakpoints).map(([key, value]) => `${key}=${value}`).join(", ")}

## Accessibility Rules
- Contrast target: ${system.foundations.accessibility.contrastTarget}
- Focus treatment: ${system.foundations.accessibility.focusTreatment}
- Keyboard pattern: ${system.foundations.accessibility.keyboardPattern}
- Touch target minimum spacing key: ${system.foundations.accessibility.touchTargetMin}
- Screen reader label prefix: ${system.foundations.accessibility.screenReaderLabelPrefix}

## Component Recipe Summary
${componentSummaryTable(system)}

## Claude Implementation Guidance
- When asked to build UI, start from semantic tokens and component recipes.
- Use raw palette scales only when semantic roles are insufficient.
- Keep reusable components token-driven and theme-aware.
- Do not hard-code brand colors when a token exists.
- Do not introduce new radius, spacing, shadow, or type scales without explicit approval.
- If a requested design conflicts with contrast or accessibility, flag it and ask whether to force the design or adjust it.
- If a component is missing, create it using the closest existing recipe family and document the new recipe.

## Suggested Files For A New Project
- \`design.md\`: This Claude-facing design instruction file.
- \`tokens.json\`: Structured design tokens.
- \`components.json\`: Component recipes and behavior defaults.
- \`theme.css\`: Runtime CSS variables.
- \`theme-layer.css\`: Theme layer mapping.
`;
}

export async function buildZip(system: GeneratedSystem, brandName: string) {
  const zip = new JSZip();
  zip.file("tokens.json", buildTokensJson(system, brandName));
  zip.file("components.json", buildComponentsJson(system, brandName));
  zip.file("theme.css", buildThemeCss(system));
  zip.file("theme-layer.css", buildThemeLayerCss(system));
  zip.file("README.md", buildReadme(system, brandName));
  zip.file("design.md", buildDesignMd(system, brandName));
  zip.file("design-system-session.json", buildSessionJson(system, brandName));

  return zip.generateAsync({ type: "blob" });
}

export function downloadTextFile(filename: string, contents: string, mimeType: string) {
  const blob = new Blob([contents], { type: mimeType });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = filename;
  anchor.click();
  URL.revokeObjectURL(url);
}

export function downloadBlob(filename: string, blob: Blob) {
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = filename;
  anchor.click();
  URL.revokeObjectURL(url);
}
